...
public class UniversalProductParser implements ProductParser {
  ...
  private static final Pattern PRICE_PATTERN =
      Pattern.compile("([0-9]+[\\.,]?[0-9]*)\\s*([рруб])?", Pattern.CASE_INSENSITIVE);
  ...
  public Product parse(String url) {
    try {
      ...
      Document doc = Jsoup.connect(url)
          .userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36").timeout(10000)
          .get();
  ...
  private String extractDescription(Document doc) {
    ...
    Element descElement =
        doc.select(".description, .product-description, [class*=description]").first();
    if (descElement != null) {
